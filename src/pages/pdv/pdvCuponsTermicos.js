...
function blocoFinanceiro({
  ...
  const rotuloDesc = cupomCodigo
    ? `Desconto (cupom ${String(cupomCodigo).toUpperCase()})`
    : "Desconto";
  return `
    ${sep()}
    <div class="row"><span>Subtotal</span><span>${moeda(subtotal)}</span></div>
    ${desconto > 0 ? `<div class="row"><span>${esc(rotuloDesc)}</span><span>-${moeda(desconto)}</span></div>` : ""}
...
export function htmlCupomClienteSimplificado(ctx = {}) {
  ...
  return `
    ...
    ${blocoFinanceiro({ subtotal, desconto, cupomCodigo, total, labelTotal: "TOTAL" })}
