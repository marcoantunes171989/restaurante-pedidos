...
export default function LandingAnalyticsAdmin() {
  ...
  return <div className="space-y-5 pb-8">
    <header className="flex flex-col justify-between gap-4 rounded-3xl bg-[#012E46] p-5 text-white sm:flex-row sm:items-center">
      <div><p className="text-xs font-black uppercase tracking-[.18em] text-[#F38525]">Super Admin</p><h1 className="mt-1 text-2xl font-black">Audiência da landing page</h1><p className="mt-1 text-sm text-white/70">Acessos a www.pedidoprime.com.br por dispositivo, navegador e localização aproximada.</p></div>
      <div className="flex gap-2">{periods.map((p) => <button key={p.id} onClick={() => setDays(p.id)} className={`rounded-xl px-3 py-2 text-xs font-bold ${days === p.id ? "bg-[#F38525] text-[#012E46]" : "bg-white/10 text-white hover:bg-white/20"}`}>{p.label}</button>)}</div>
    </header>
    ...
    {loading ? <div className="rounded-2xl bg-white p-8 text-center text-[#637985]">Carregando indicadores…</div> : data ? <>
      ...
      <section className="rounded-2xl border border-[#DDE4E8] bg-white p-4"><div className="flex items-center gap-2"><BarChart3 className="h-5 w-5 text-[#F38525]" /><h3 className="font-black text-[#012E46]">Visualizações por dia</h3></div><div className="mt-5 flex h-44 items-end gap-1.5">{data.byDay.map((r) => <div key={r.date} className="group flex min-w-0 flex-1 flex-col items-center justify-end gap-1" title={`${r.date}: ${r.value}`}><span className="hidden text-[9px] font-bold text-[#012E46] group-hover:block">{r.value}</span><div className="w-full rounded-t bg-[#F38525]" style={{ height: `${Math.max(4, r.value / peak * 100)}%` }} /><span className="hidden text-[8px] text-[#637985] md:block">{r.date.slice(5)}</span></div>)}</div></section>
