...
import { BarChart3, CalendarDays, Clock3, Eye, MapPin, MonitorSmartphone, Trash2, Users } from "lucide-react";
...
export default function LandingAnalyticsAdmin() {
  ...
  const load = useCallback(async ({ silent = false } = {}) => {
    ...
    try {
      ...
      const response = await fetch(`/api/landing-analytics?from=${encodeURIComponent(from)}&page=${page}`, { headers: { authorization: `Bearer ${token}` }, cache: "no-store" });
