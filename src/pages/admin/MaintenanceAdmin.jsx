...
import {
  ...
  Wrench, GitCommit, Timer, Megaphone, PlayCircle,
} from "lucide-react";
...
export default function MaintenanceAdmin() {
  ...
  if (sessaoStatus === "indisponivel") {
    return (
      <div className="mx-auto max-w-5xl space-y-5 px-1 pb-8">
        <PageHeader
          icone={<Wrench className="h-5 w-5" aria-hidden="true" />}
          titulo="Manutenção"
          descricao="Orquestração de manutenção do Maintenance Write Fence."
        />
  ...
  return (
    <div className="mx-auto max-w-5xl space-y-5 px-1 pb-8">
      <PageHeader
        icone={<Wrench className="h-5 w-5" aria-hidden="true" />}
        titulo="Manutenção"
        descricao="Orquestração do Maintenance Write Fence — início de ciclo e aviso público."
        indicadores={[
