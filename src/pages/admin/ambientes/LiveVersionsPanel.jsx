...
// ════════════════════════════════════════════════════════════
//  Versões e deploys (ao vivo) — Microgate 23 + Microgate 03
//  Painel LEGADO da tela Ambientes & Releases, preservado sem mudança de
//  comportamento na PDB-I3-FE1 (só o cabeçalho de página subiu para o shell
//  AmbientesAdmin.jsx; aqui ficou uma barra de status + "Atualizar agora").
...
// Barra de status do painel ao vivo (o título da página fica no shell).
function BarraStatusAoVivo({ indicadores, acao }) {
  return (
      ...
      <div className="flex flex-wrap items-center gap-x-2 gap-y-1.5">
        {indicadores.map((ind, i) => (
          <span key={i} className="flex items-center gap-2">
            ...
            <span className={`text-xs font-semibold ${ind.tom === "ok" ? "text-[#2F9E52]" : ind.tom === "alerta" ? "text-[#F38525]" : ind.tom === "erro" ? "text-[#C81E4A]" : "text-[#6B7280]"}`}>
              <b className="font-bold text-[#111111]">{ind.valor}</b> {ind.rotulo}
            </span>
          </span>
        ))}
      </div>
...
export default function LiveVersionsPanel() {
