  ...
  return (
    <div ref={containerRef} className="flex h-full flex-col bg-[var(--client-surface)]">
              ...
              {item.comboId ? (
                ...
                <div className="flex items-center gap-1 rounded-lg border border-[var(--client-border)] bg-[var(--client-background-soft)] p-0.5">
                  ...
                  <button type="button" onClick={() => addToCart(item)} aria-label={`Adicionar mais uma unidade de ${item.name}`}
                    className="flex h-7 w-7 items-center justify-center rounded-md bg-[var(--client-primary)] text-white transition hover:bg-[var(--client-primary-hover)]"><Plus aria-hidden="true" size={13} /></button>
                </div>
              )}
            </div>
      ...
      <div className="shrink-0 space-y-3 border-t border-[var(--client-border)] bg-[var(--client-background-soft)] px-4 py-3.5">
        ...
        {!comandaValida ? (
          <button type="button" onClick={onAbrirScanner} disabled={!podeEscanear}
            className="flex h-12 w-full items-center justify-center gap-2 rounded-xl bg-[var(--client-primary)] text-sm font-bold text-white transition active:scale-[0.98] hover:bg-[var(--client-primary-hover)] disabled:cursor-not-allowed disabled:bg-[var(--client-disabled-background)] disabled:text-[var(--client-disabled-text)]">
