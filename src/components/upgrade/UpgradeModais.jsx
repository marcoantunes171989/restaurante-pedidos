...
export function PlanosModal({ onFechar, onEscolherPlano, onFalarConsultor }) {
  return (
    <div className={overlay} onClick={onFechar}>
      <div onClick={(e) => e.stopPropagation()} className={`${cardModal} max-w-6xl`} style={{ fontFamily: "'Inter','Sora',sans-serif" }}>
        <div className="relative shrink-0 px-6 pt-6 text-center">
          <button onClick={onFechar} className={`${btnFechar} absolute right-5 top-5`}>✕</button>
        ...
        <div className="flex-1 overflow-y-auto px-6 py-5">
          <div className="grid gap-4 lg:grid-cols-4">
            {planosPedidoPrime.map((p) => {
              ...
              return (
                <div key={p.id} className={`relative flex flex-col rounded-[1.6rem] border bg-[#0B0F1A] p-5 transition ${p.destaque ? "border-gold-400/70 shadow-[0_0_0_1px_rgba(214,168,79,.4),0_16px_44px_-12px_rgba(214,168,79,.3)]" : "border-gold-400/15"}`}>
