...
export default function Fluxo() {
  return (
    <section id="como-funciona" className="scroll-mt-24 bg-[var(--pp-bg)] py-16 sm:py-24">
      <div className="mx-auto max-w-7xl px-5">
        ...
        <div className="mt-16">
          <Reveal className="mx-auto max-w-3xl text-center">
            <h3 className="font-display text-xl font-bold text-[var(--pp-graphite)]">Operação integrada, do pedido ao resultado</h3>
          </Reveal>
            ...
            <div className="mx-auto flex w-max min-w-full items-center justify-center gap-2 px-1 sm:gap-3">
              {FLUXO_OPERACAO.map((etapa, i) => (
                <div key={etapa} className="flex items-center gap-2 sm:gap-3">
                  <span className="whitespace-nowrap rounded-full border border-[var(--pp-border)] bg-white px-4 py-2.5 text-xs font-bold text-[var(--pp-graphite)] shadow-sm sm:text-sm">{etapa}</span>
                  {i < FLUXO_OPERACAO.length - 1 && <Icone nome="crescimento" className="h-4 w-4 shrink-0 rotate-[-35deg] text-[var(--pp-border)]" />}
                </div>
              ))}
            </div>
          </Reveal>
          <div className="mt-8 grid gap-4 sm:grid-cols-3">
            {FLUXO_BENEFICIOS.map((b, i) => (
              <Reveal as="article" key={b.titulo} delay={i * 80} className="rounded-[1.25rem] border border-[var(--pp-border)] bg-white p-5 text-center">
                <div className="flex justify-center"><IconBadge nome={b.icon} tom="green" /></div>
                <h4 className="font-display mt-3 text-sm font-bold text-[var(--pp-graphite)]">{b.titulo}</h4>
                <p className="mt-1 text-xs leading-5 text-[var(--pp-text-muted)]">{b.desc}</p>
              </Reveal>
            ))}
          </div>
        </div>
