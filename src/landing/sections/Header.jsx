...
export default function Header({ onEntrar }) {
  ...
  return (
    <header className={`sticky top-0 z-50 border-b bg-white/85 backdrop-blur-xl transition-shadow ${rolado ? "border-[#E2E8F0] shadow-[0_4px_20px_rgba(13,27,42,0.06)]" : "border-transparent"}`} style={{ paddingTop: "env(safe-area-inset-top)" }}>
      <nav className="mx-auto flex max-w-7xl items-center justify-between gap-4 px-5 py-3.5">
        ...
        <div className="hidden items-center gap-1 lg:flex">
          {NAV.map((n) => (
            <button key={n.id} onClick={() => irPara(n.id)}
              className="rounded-lg px-3 py-2 text-sm font-semibold text-[#334155] transition hover:bg-[#F8FAFC] hover:text-[#0D1B2A]">
              {n.label}
            </button>
          ))}
        </div>
        <div className="hidden items-center gap-2 md:flex">
          <button onClick={onEntrar} className="rounded-xl px-3.5 py-2.5 text-sm font-bold text-[#334155] transition hover:bg-[#F8FAFC]">Entrar</button>
        ...
        <button onClick={() => setMenuAberto((a) => !a)} aria-label={menuAberto ? "Fechar menu" : "Abrir menu"} aria-expanded={menuAberto}
          className="flex h-11 w-11 items-center justify-center rounded-xl border border-[#E2E8F0] bg-white text-[#0D1B2A] lg:hidden">
      ...
      {menuAberto && (
        <div className="border-t border-[#E2E8F0] bg-white px-5 pb-5 pt-2 lg:hidden">
          <div className="grid gap-1">
            {NAV.map((n) => (
              <button key={n.id} onClick={() => irPara(n.id)}
                className="min-h-[44px] rounded-xl px-3 py-2.5 text-left text-sm font-semibold text-[#334155] transition hover:bg-[#F8FAFC] hover:text-[#0D1B2A]">
                {n.label}
              </button>
            ))}
          </div>
          <div className="mt-3 grid gap-2">
            <button onClick={onEntrar} className="min-h-[44px] w-full rounded-xl border border-[#E2E8F0] px-4 py-2.5 text-sm font-bold text-[#334155] transition hover:bg-[#F8FAFC]">Entrar</button>
