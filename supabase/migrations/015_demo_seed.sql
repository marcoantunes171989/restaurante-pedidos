-- ============================================================
-- MIGRATION 015 — Limpeza + Seed de demonstração (4 segmentos)
-- Execute no: Supabase Dashboard → SQL Editor → New Query
-- PRÉ-REQUISITO: migrations 001..014 já executadas.
-- ⚠️ APAGA todas as empresas exceto a Matriz (prefixo CMD) e seus dados.
-- ============================================================

-- ─────────────────────────────────────────────────────────────
-- 1) LIMPEZA — mantém apenas a Matriz (CMD) e seus produtos/usuários
-- ─────────────────────────────────────────────────────────────
DO $$
DECLARE v_matriz bigint;
BEGIN
  SELECT id INTO v_matriz FROM tab_lojas WHERE prefixo = 'CMD' LIMIT 1;
  IF v_matriz IS NULL THEN
    RAISE EXCEPTION 'Empresa Matriz (prefixo CMD) não encontrada. Verifique a migration 011.';
  END IF;

  DELETE FROM tab_pedidos          WHERE loja_id IS NOT NULL AND loja_id <> v_matriz;
  DELETE FROM tab_produtos         WHERE loja_id IS NOT NULL AND loja_id <> v_matriz;
  DELETE FROM tab_categorias       WHERE loja_id IS NOT NULL AND loja_id <> v_matriz;
  DELETE FROM tab_formas_pagamento WHERE loja_id IS NOT NULL AND loja_id <> v_matriz;
  DELETE FROM tab_usuarios         WHERE loja_id IS NOT NULL AND loja_id <> v_matriz AND super_admin = false;
  DELETE FROM tab_lojas            WHERE id <> v_matriz;
END $$;

-- ─────────────────────────────────────────────────────────────
-- 2) Função auxiliar para criar empresa + cargos/formas/usuários
-- ─────────────────────────────────────────────────────────────
...
-- Função auxiliar para inserir produto
...
-- ─────────────────────────────────────────────────────────────
-- 4) Limpeza das funções auxiliares
-- ─────────────────────────────────────────────────────────────
