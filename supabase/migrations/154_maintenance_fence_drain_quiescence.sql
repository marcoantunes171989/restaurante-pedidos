-- ════════════════════════════════════════════════════════════
--  154 — Fence / Drain / Quiescence (B13-A2).
--
--  Congelado em B13-A1R1. Sem CREATE/DROP TABLE, sem coluna,
--  índice, constraint, policy ou TABLE ACL. Sem novo event_type.
--  Sem mutação de app_release_runs. Sem editar 140–153.
--
--  Novas privadas (3):
...
--  CREATE OR REPLACE (3), contratos públicos preservados:
--    app_assert_business_write_allowed(uuid, text)
--    app_maintenance_operation_begin_internal(text)
--    app_checkout_begin(bigint, text[])
--
--  Barreira advisory xact (154, 1): shared no begin/checkout;
--  exclusive em fence/drain_start/quiesce/probe/release_start.
--  Finish/cancel/commit NÃO adquirem a barreira.
-- ════════════════════════════════════════════════════════════
...
begin
  if to_regclass('public.app_maintenance_state') is null then
    raise exception 'precheck 154: public.app_maintenance_state não existe (migration 140 ausente).';
  end if;
  if to_regclass('public.app_maintenance_operations') is null then
    raise exception 'precheck 154: public.app_maintenance_operations não existe (migration 141 ausente).';
  end if;
  if to_regclass('public.app_maintenance_events') is null then
    raise exception 'precheck 154: public.app_maintenance_events não existe (migration 140 ausente).';
  end if;
  if to_regclass('public.app_release_runs') is null then
    raise exception 'precheck 154: public.app_release_runs não existe (migration 138 ausente).';
  end if;
  if to_regclass('public.tab_pedidos') is null then
    raise exception 'precheck 154: public.tab_pedidos não existe.';
  end if;
  ...
  if v_condef is distinct from v_event_type_condef then
    raise exception 'precheck 154: app_maintenance_events_event_type_check divergente do contrato canônico de 17 valores (drift): %', v_condef;
  end if;

  v_assert_oid := to_regprocedure('public.app_assert_business_write_allowed(uuid, text)');
  if v_assert_oid is null then
    raise exception 'precheck 154: public.app_assert_business_write_allowed(uuid, text) não existe (migration 142 ausente).';
  end if;
  ...
  if v_owner is distinct from 'postgres' then
    raise exception 'precheck 154: app_assert_business_write_allowed — owner deveria ser postgres (owner atual: %).', coalesce(v_owner, 'NULL');
  end if;
  if not coalesce(v_prosecdef, false) then
    raise exception 'precheck 154: app_assert_business_write_allowed — deveria ser SECURITY DEFINER.';
  end if;
  if v_proconfig is null or not ('search_path=public' = any (v_proconfig)) then
    raise exception 'precheck 154: app_assert_business_write_allowed — proconfig deveria conter search_path=public.';
  end if;
  if v_prorettype is distinct from 'void'::regtype then
    raise exception 'precheck 154: app_assert_business_write_allowed — return type deveria ser void.';
  end if;
  if v_pronargs is distinct from 2 or v_pronargdefaults is distinct from 2 then
    raise exception 'precheck 154: app_assert_business_write_allowed — assinatura deveria ser (uuid, text) com 2 defaults.';
  end if;

  v_begin_oid := to_regprocedure('public.app_maintenance_operation_begin_internal(text)');
  if v_begin_oid is null then
    raise exception 'precheck 154: public.app_maintenance_operation_begin_internal(text) não existe (migration 150 ausente).';
  end if;
  ...
  if v_owner is distinct from 'postgres' then
    raise exception 'precheck 154: app_maintenance_operation_begin_internal — owner deveria ser postgres (owner atual: %).', coalesce(v_owner, 'NULL');
  end if;
  if not coalesce(v_prosecdef, false) then
    raise exception 'precheck 154: app_maintenance_operation_begin_internal — deveria ser SECURITY DEFINER.';
  end if;
  if v_proconfig is null or not ('search_path=public' = any (v_proconfig)) then
    raise exception 'precheck 154: app_maintenance_operation_begin_internal — proconfig deveria conter search_path=public.';
  end if;
  if v_prorettype is distinct from 'uuid'::regtype then
    raise exception 'precheck 154: app_maintenance_operation_begin_internal — return type deveria ser uuid.';
  end if;

  v_checkout_oid := to_regprocedure('public.app_checkout_begin(bigint, text[])');
  if v_checkout_oid is null then
    raise exception 'precheck 154: public.app_checkout_begin(bigint, text[]) não existe (migration 152 ausente).';
  end if;
  select p.prosecdef, p.proconfig, p.prorettype, pg_get_userbyid(p.proowner)
    into v_prosecdef, v_proconfig, v_prorettype, v_owner
  from pg_proc p where p.oid = v_checkout_oid;
  if v_owner is distinct from 'postgres' then
    raise exception 'precheck 154: app_checkout_begin — owner deveria ser postgres (owner atual: %).', coalesce(v_owner, 'NULL');
  end if;
  if not coalesce(v_prosecdef, false) then
    raise exception 'precheck 154: app_checkout_begin — deveria ser SECURITY DEFINER.';
  end if;
  if v_proconfig is null or not ('search_path=public' = any (v_proconfig)) then
    raise exception 'precheck 154: app_checkout_begin — proconfig deveria conter search_path=public.';
  end if;
  if v_prorettype is distinct from 'jsonb'::regtype then
    raise exception 'precheck 154: app_checkout_begin — return type deveria ser jsonb.';
  end if;
  ...
  if v_transition_oid is null then
    raise exception 'precheck 154: public.app_maintenance_orchestration_transition_internal não existe (migration 153 ausente).';
  end if;
  ...
  if exists (
    ...
    raise exception 'precheck 154: colisão — alguma das 8 funções novas do B13 já existe.';
  end if;
end $$;

-- ════════════════════════════════════════════════════════════
--  1) PRIVATE — cutover barrier (chave fixa 154, 1)
-- ════════════════════════════════════════════════════════════
create function public.app_maintenance_cutover_barrier_internal(
  p_exclusive boolean
)
returns void
language plpgsql
...
-- ════════════════════════════════════════════════════════════
--  2) PRIVATE — drain IN_FLIGHT count (N-1, N)
-- ════════════════════════════════════════════════════════════
create function public.app_maintenance_drain_in_flight_count_internal()
returns integer
language plpgsql
volatile
security definer
...
-- ════════════════════════════════════════════════════════════
--  3) PRIVATE — expire stale IN_FLIGHT (N-1, N)
-- ════════════════════════════════════════════════════════════
create function public.app_maintenance_operation_expire_internal()
returns void
language plpgsql
volatile
security definer
...
-- ════════════════════════════════════════════════════════════
--  4) REPLACE — app_assert_business_write_allowed
-- ════════════════════════════════════════════════════════════
...
-- ════════════════════════════════════════════════════════════
--  5) REPLACE — app_maintenance_operation_begin_internal
-- ════════════════════════════════════════════════════════════
...
-- ════════════════════════════════════════════════════════════
--  6) REPLACE — app_checkout_begin (shared antes de tab_pedidos)
-- ════════════════════════════════════════════════════════════
...
-- ════════════════════════════════════════════════════════════
--  7) PUBLIC RPC — fence (NOTICE -> FENCING)
-- ════════════════════════════════════════════════════════════
create function public.app_maintenance_orchestration_fence(
...
-- ════════════════════════════════════════════════════════════
--  8) PUBLIC RPC — drain_start (FENCING -> DRAINING)
-- ════════════════════════════════════════════════════════════
create function public.app_maintenance_orchestration_drain_start(
  p_expected_version integer,
  p_actor_user_id uuid,
  p_actor_email text,
  p_reason text,
...
-- ════════════════════════════════════════════════════════════
--  9) PUBLIC RPC — quiesce (DRAINING -> QUIESCENT se count=0)
-- ════════════════════════════════════════════════════════════
create function public.app_maintenance_orchestration_quiesce(
  p_expected_version integer,
  p_actor_user_id uuid,
  p_actor_email text,
  p_reason text,
...
-- ════════════════════════════════════════════════════════════
--  10) PUBLIC RPC — quiescence_probe (não é edge estrutural)
-- ════════════════════════════════════════════════════════════
create function public.app_maintenance_orchestration_quiescence_probe(
  p_expected_version integer,
  p_actor_user_id uuid,
  p_actor_email text,
  p_reason text,
...
-- ════════════════════════════════════════════════════════════
--  11) PUBLIC RPC — release_start (QUIESCENT -> RELEASING)
-- ════════════════════════════════════════════════════════════
create function public.app_maintenance_orchestration_release_start(
  p_expected_version integer,
  p_actor_user_id uuid,
  p_actor_email text,
  p_reason text,
...
declare
  ...
  v_new_private_count integer;
  v_new_public_count integer;
begin
  v_events_reloid := to_regclass('public.app_maintenance_events');
  select pg_get_constraintdef(oid) into v_condef
  from pg_constraint
  where conrelid = v_events_reloid and conname = 'app_maintenance_events_event_type_check';
  if v_condef is distinct from v_event_type_condef then
    raise exception 'postcheck 154: app_maintenance_events_event_type_check divergente do contrato canônico de 17 valores: %', v_condef;
  end if;

  select count(*) into v_new_private_count
  from pg_proc p
  join pg_namespace n on n.oid = p.pronamespace
  where n.nspname = 'public'
    and p.proname in (
  ...
  if v_new_private_count <> 3 then
    raise exception 'postcheck 154: esperado exatamente 3 funções privadas novas (count=%).', v_new_private_count;
  end if;

  select count(*) into v_new_public_count
  from pg_proc p
  join pg_namespace n on n.oid = p.pronamespace
  where n.nspname = 'public'
    and p.proname in (
  ...
  if v_new_public_count <> 5 then
    raise exception 'postcheck 154: esperado exatamente 5 RPCs públicas novas (count=%).', v_new_public_count;
  end if;

  -- Privadas novas: sem EXECUTE para clientes/service_role.
  ...
  loop
    if v_oid is null then
      raise exception 'postcheck 154: função privada nova não encontrada.';
    end if;
    select p.prosecdef, p.proconfig, pg_get_userbyid(p.proowner)
      into v_prosecdef, v_proconfig, v_owner
    from pg_proc p where p.oid = v_oid;
    if v_owner is distinct from 'postgres' then
      raise exception 'postcheck 154: privada — owner deveria ser postgres.';
    end if;
    if not coalesce(v_prosecdef, false) then
      raise exception 'postcheck 154: privada — deveria ser SECURITY DEFINER.';
    end if;
    if v_proconfig is null or not ('search_path=public' = any (v_proconfig)) then
      raise exception 'postcheck 154: privada — proconfig deveria conter search_path=public.';
    end if;
    if has_function_privilege('anon', v_oid, 'execute')
       or has_function_privilege('authenticated', v_oid, 'execute')
       or has_function_privilege('service_role', v_oid, 'execute') then
      raise exception 'postcheck 154: privada NÃO deveria ter EXECUTE para anon/authenticated/service_role.';
    end if;
    select exists (
      select 1
      from pg_proc p
    ...
    if v_public_execute then
      raise exception 'postcheck 154: privada — PUBLIC (grantee=0) NÃO deveria ter EXECUTE.';
    end if;
  end loop;

  -- Públicas novas: GRANT EXECUTE somente service_role.
  foreach v_oid in array array[
  ...
  loop
    if v_oid is null then
      raise exception 'postcheck 154: RPC pública nova não encontrada.';
    end if;
    select p.prosecdef, p.proconfig, p.prorettype, pg_get_userbyid(p.proowner)
      into v_prosecdef, v_proconfig, v_prorettype, v_owner
    from pg_proc p where p.oid = v_oid;
    if v_owner is distinct from 'postgres' then
      raise exception 'postcheck 154: pública — owner deveria ser postgres.';
    end if;
    if not coalesce(v_prosecdef, false) then
      raise exception 'postcheck 154: pública — deveria ser SECURITY DEFINER.';
    end if;
    if v_proconfig is null or not ('search_path=public' = any (v_proconfig)) then
      raise exception 'postcheck 154: pública — proconfig deveria conter search_path=public.';
    end if;
    if v_prorettype is distinct from 'void'::regtype then
      raise exception 'postcheck 154: pública — return type deveria ser void.';
    end if;
    if has_function_privilege('anon', v_oid, 'execute')
       or has_function_privilege('authenticated', v_oid, 'execute') then
      raise exception 'postcheck 154: pública NÃO deveria ter EXECUTE para anon/authenticated.';
    end if;
    if not has_function_privilege('service_role', v_oid, 'execute') then
      raise exception 'postcheck 154: pública deveria ter EXECUTE para service_role.';
    end if;
  end loop;

  -- Contratos replaced preservados.
  v_oid := to_regprocedure('public.app_assert_business_write_allowed(uuid, text)');
  if v_oid is null then
    raise exception 'postcheck 154: app_assert_business_write_allowed(uuid, text) não encontrada.';
  end if;
  select p.prosecdef, p.proconfig, p.prorettype, p.pronargs, p.pronargdefaults, pg_get_userbyid(p.proowner)
    into v_prosecdef, v_proconfig, v_prorettype, v_pronargs, v_pronargdefaults, v_owner
  from pg_proc p where p.oid = v_oid;
  if v_owner is distinct from 'postgres'
    ...
    raise exception 'postcheck 154: app_assert_business_write_allowed — contrato quebrado.';
  end if;
  if has_function_privilege('anon', v_oid, 'execute')
     or has_function_privilege('authenticated', v_oid, 'execute')
     or has_function_privilege('service_role', v_oid, 'execute') then
    raise exception 'postcheck 154: app_assert_business_write_allowed NÃO deveria ter EXECUTE para clientes/service_role.';
  end if;

  v_oid := to_regprocedure('public.app_maintenance_operation_begin_internal(text)');
  if v_oid is null then
    raise exception 'postcheck 154: app_maintenance_operation_begin_internal(text) não encontrada.';
  end if;
  select p.prosecdef, p.proconfig, p.prorettype, pg_get_userbyid(p.proowner)
    into v_prosecdef, v_proconfig, v_prorettype, v_owner
  from pg_proc p where p.oid = v_oid;
  if v_owner is distinct from 'postgres'
     ...
     or v_proconfig is null
     or not ('search_path=public' = any (v_proconfig)) then
    raise exception 'postcheck 154: app_maintenance_operation_begin_internal — contrato quebrado.';
  end if;
  if has_function_privilege('anon', v_oid, 'execute')
     or has_function_privilege('authenticated', v_oid, 'execute')
     or has_function_privilege('service_role', v_oid, 'execute') then
    raise exception 'postcheck 154: begin_internal NÃO deveria ter EXECUTE para clientes/service_role.';
  end if;

  v_oid := to_regprocedure('public.app_checkout_begin(bigint, text[])');
  if v_oid is null then
    raise exception 'postcheck 154: app_checkout_begin(bigint, text[]) não encontrada.';
  end if;
  select p.prosecdef, p.proconfig, p.prorettype, pg_get_userbyid(p.proowner)
    into v_prosecdef, v_proconfig, v_prorettype, v_owner
  from pg_proc p where p.oid = v_oid;
  if v_owner is distinct from 'postgres'
     ...
     or v_proconfig is null
     or not ('search_path=public' = any (v_proconfig)) then
    raise exception 'postcheck 154: app_checkout_begin — contrato quebrado.';
  end if;
  if has_function_privilege('anon', v_oid, 'execute')
     or has_function_privilege('service_role', v_oid, 'execute') then
    raise exception 'postcheck 154: app_checkout_begin NÃO deveria ter EXECUTE para anon/service_role.';
  end if;
  if not has_function_privilege('authenticated', v_oid, 'execute') then
    raise exception 'postcheck 154: app_checkout_begin deveria preservar EXECUTE para authenticated.';
  end if;
end $$;
